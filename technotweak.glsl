...
vec3 grayify(vec3 col){
    return vec3(col.r/col.g*col.b); //CHANGE HERE (integer and operations btw colors)
    }

void main() {
    float amp = spectrum.y * 20 + 1;
    vec2 st = gl_FragCoord.xy/resolution;
    vec3 pct = vec3(st.x);
    vec3 color = vec3(0.0);

    vec2 toCenter = vec2(0.5)-st + vec2(cnoise(vec3(time/15.0)), cnoise(vec3(time/2.0)));
    float angle = atan(toCenter.y,toCenter.x);
    float radius = length(toCenter)*7.0;
    float hue = ((angle/(PI*2.0)* amp)+ 0.5 * 100.0);
    float brightness = ((angle/(PI*1.0)* 10* amp));
    color = hsb2rgb(vec3(radius + amp/10.0, hue * 3.0, sin(brightness)*0.05));
    color = grayify(color);
