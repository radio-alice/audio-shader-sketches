...
void main() {
    float amp = spectrum.y * 60 + 1;
    vec2 st = gl_FragCoord.xy/resolution;
    vec3 pct = vec3(st.x);
    vec3 color = vec3(0.0);

    vec2 toCenter = vec2(0.5)-st + vec2(cnoise(vec3(time/15.0)), cnoise(vec3((time+20.0)/12.0)))*0.5;
    float angle = atan(toCenter.y+ cnoise(vec3((time+300.0)/8.0)),toCenter.x + cnoise(vec3(time/9.0)));
    float radius = length(toCenter)*7.0;
    float hue = ((angle/(PI*2.0)* amp)+ 0.5 *time/10.0);
    float brightness = ((angle/(PI*1.0)* amp) + 0.5 * 100.0);
    color = hsb2rgb(vec3(hue, radius, radius));
    color = vec3(color.r*color.g-color.b - amp/20.0); //Change operations here
    color += texture(prevFrame, st).r * min(mouse.x * 100, 0.8 + amp); //Change here (1++)
