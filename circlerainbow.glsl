...
void main() {
    float amp = spectrum.y * 40 + 1;
    vec2 st = gl_FragCoord.xy/resolution;
    vec3 pct = vec3(st.x);
    vec3 color = vec3(0.0);

    vec2 toCenter = vec2(0.5)-st + vec2(cnoise(vec3((time+20)/10.0)), cnoise(vec3(time/10.0)));
    float angle = atan(toCenter.y,toCenter.x);
    float radius = length(toCenter)*6.0 + amp/15;
    float hue = ((angle/(PI*2.0)* amp)+ 0.5 *time/10.0);
    float brightness = ((angle/(PI*1.0)* amp) + 0.5 * 10000.0);
